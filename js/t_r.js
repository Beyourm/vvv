...
// تم نقل الرابط الى ملف الروابط 

const scriptURL = "https://script.google.com/macros/s/AKfycbzMGm6kLbx4qhWIgw3_z9djnDGHhHFUniYe55KmCxvNdqnVwp1wBwC2nA3U_NtBnv6O/exec";
...
// ------------------------------------------------------------------
// دوال التحكم في نافذة النجاح المتحركة (Success Modal) - المعدلة
// ------------------------------------------------------------------
...
function showSuccessDialog() {
    ...
    // 2. إزالة فصول الأنيميشن أولاً (لإعادة تشغيل الأنيميشن بشكل سليم في كل مرة)
    circle.classList.remove('draw');
    tick.classList.remove('draw');
    
    // 3. تفعيل الأنيميشن بتأخير بسيط لـ 'Reflow' (تشغيل التسلسل)
    //    الـ CSS هو الذي سيتحكم بالتسلسل الزمني (الدائرة 1.2 ثانية، الصح يبدأ بعد 1.2 ثانية)
    setTimeout(() => {
...
function closeDialog(){
    ...
    // إزالة فصول الأنيميشن عند الإغلاق لتهيئتها للعرض التالي
    circle.classList.remove('draw');
    tick.classList.remove('draw');
    
    overlay.classList.remove("show"); // إخفاء النافذة
    
    // التوجيه للرئيسية عند الضغط على زر الموافقة
    window.location.href = "index.html"; 
}

// ⚠️ الإضافة الضرورية: ربط الزر closeDialogBtn بدالة closeDialog
